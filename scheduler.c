...
int avg_wt[8] ={0,0,0,0,0,0,0,0}, avg_tat[8] ={0,0,0,0,0,0, 0,0};
...
void calAvg(int processes[][7],int n, int a ){
	...
	for (int i=0; i<n; i++)
	{	total_wt = total_wt + processes[i][3];
	total_tat = total_tat + processes[i][4];
	}
	...
	printf("Average waiting time = %d",avg_wt[a]);
	printf("\n");
	printf("Average turn around time = %d \n",avg_tat[a]);
}

void ff( int processes[][7], int n)
{
	processes[0][3]=0;//wt
	processes[0][4]=processes[0][2];//tat
	processes[0][5]=processes[0][1]+processes[0][2]+processes[0][3];//ct
	for(int i=1; i<n;i++){
	...
	printf("\n\n----------------------------------------------\n");
	printf("PID\tArrival\tBurst\t Wait\t TAT\t CT \t \n");
	printf("\n----------------------------------------------\n");
